...
def api_home(request):
    ...
    categories=Category.objects.filter(country__name=request.GET['country'])
...
def api_cats(request):
    cats=Category.objects.filter(country__name=request.GET['country'])
    categories_serial=CategoryCountrySerializer(cats,many=True,context={"request":request})
...
def api_brands(request):
    ...
    brands2=brands.filter(country__name=request.GET['country'])
    brands_serial=BrandCountrySerializer(brands2,many=True,context={"request":request})
    print(brands_serial.data)
